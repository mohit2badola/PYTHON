...
// ---------------- INITIALIZATION ----------------
async function init() {
    showLoading("Welcome to TaskFlow Pro...");
    await fetchTasks();
    await loadStats();
    setupEventListeners();
...
function showLoading(message = "Loading...") {
    ...
    if (loadingCount === 1) {
        ...
        // Update message if multiple loadings
    ...
    for (let i = 0; i < 3; i++) {
        tbody.innerHTML += `
            <tr class="skeleton-row">
                ...
                <td><div class="skeleton-text skeleton-date"></div></td>
...
async function checkAdminStatus() {
    try {
        let res = await fetch("/admin/stats", { credentials: 'same-origin' });
        if (res.ok) {
...
function filterAndDisplayTasks() {
    ...
    filtered.sort((a, b) => {
        if (currentFilter.sortBy === 'priority') {
            ...
            let dateA = a.due_date || '9999-12-31';
            let dateB = b.due_date || '9999-12-31';
            return dateA.localeCompare(dateB);
        } else if (currentFilter.sortBy === 'created') {
...
async function toggleStatus(id, currentStatus) {
    ...
    showLoading(`Marking task as ${newStatus}...`);
...
function displayTasks(filteredTasks) {
    ...
    filteredTasks.forEach(task => {
        ...
        if (task.due_date) {
            ...
            if (task.status !== 'Completed') {
                if (task.due_date < today) {
                    ...
                    dueDateDisplay = '⚠️ OVERDUE: ' + task.due_date;
                } else if (task.due_date === today) {
                    dueDateClass = 'due-today';
                    dueDateDisplay = '🔴 DUE TODAY: ' + task.due_date;
                } else {
                    dueDateClass = 'upcoming';
                    dueDateDisplay = '📅 ' + task.due_date;
                }
            } else {
                dueDateDisplay = '✅ Completed: ' + task.due_date;
            }
        }
        ...
        <tr class="${task.status === 'Completed' ? 'completed-row' : ''}">
            ...
            <td class="due-date-cell"><span class="due-date ${dueDateClass}">${escapeHtml(dueDateDisplay)}</span></td>
...
// ---------------- LOAD STATISTICS (UPDATES PROGRESS BAR) ----------------
async function loadStats() {
    try {
        ...
        if (dueTodayElem) dueTodayElem.innerText = stats.due_today || 0;
        ...
        if (completionRateElem) completionRateElem.innerText = `${completionRate}%`;
        if (progressFillElem) {
            progressFillElem.style.width = `${completionRate}%`;
            progressFillElem.innerHTML = `${completionRate}%`;
        }
    } catch (error) {
...
function exportTodayTasks() {
    showLoading("Preparing today's tasks export...");
...
function exportAllTasks() {
    showLoading("Preparing all tasks export...");
